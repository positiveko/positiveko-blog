...
그런데 undefined도 false이고 null도 false이지만, **0 또한 false**입니다.
...
한 개의 컴포넌트 안에서 API 3개~12개 까지 연결해야 하는 상황이라면 첫 번째의 response와 마지막 열두 번째의 response까지 텀이 발생해서 순차적으로 view가 뜨는 문제가 발생할 수 있습니다.
한 번에 모든 view를 띄울 수 없을까 고민하다가 사용하게 된 것은 Promise.all과 Promise.allSettled이었습니다.
